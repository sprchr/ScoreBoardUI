...
export default function App({ params }: { params: { id: string } }) {
  ...
  async function fetchPlayers() {
    const response = await axios.get(`https://scoreboard-backend-green.vercel.app/players/${id}`);
    const filteredPlayers =response.data.players.players.filter(
      (player: Player) => player.status === 1
    );
    setGameId(response.data._id)
    const res =  response.data.players._id
    const gameScore =  response.data.scores.filter((score:any)=>(score.gameId === res ))
    setPlayers(filteredPlayers);
    setTotal(gameScore[0].totalArray)
    setScoreId(gameScore[0]._id)
    setTempTotal(gameScore[0].totalArray)
    setRounds(gameScore[0].totalArray.length)
  }
  useEffect(() => {
    fetchPlayers();
  }, []);
  ...
  async function storeData() {
    ...
    try {
      const response = await axios.post(`https://scoreboard-backend-green.vercel.app/games/${gameId}`, {
        scoreId,
        scoreCard: tempArray, 
      });
